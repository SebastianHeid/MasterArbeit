...
In the first iteration, for all $L$ combinations of model configurations $\mathcal{M}^{(1)} = \{ (B \setminus \{B_i\}) \cup \{\hat{B}_i\} \mid 1 \leq i \leq L \}$ the metrics are computed and the best block $B_j$ is selected according to the ranking. Let $\bar{B}_j$ denote the compressed version of block $B_j$.
 ...
 $\mathcal{M}^{(2)} = \{ (B \setminus \{B_i, B_j\}) \cup \{\hat{B}_i, \bar{B}_j\} \mid 1 \leq i \neq j \leq L \}$.  For the $n^\text{th}$ iteration $L-(n-l)$ uncompressed blocks remains. Let $B_\text{comp}$ denote the set of all previously compressed blocks then the potential candidates for compression are given by  $\mathcal{M}^{(n)} = \{ (B \setminus (B_\text{comp} \cup {B_i})) \cup B_\text{comp} \cup {\hat{B}_i} \mid B_i \in B \setminus B_\text{comp} \}$.
