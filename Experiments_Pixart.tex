...
In this section the standard setting (see sec. \ref{sec:DefaultSetup}) except the learning objective is varied to investigate the effectiveness of the individual components of the knowledge distillation loss (see eq. \ref{eq:KnowledgeDistillationLoss}). In total, five different loss functions are testet: 
 ...
 As an alternative to the pruning strategy of removing entire transformer blocks from PixArt-$\Sigma$, a compression strategy utilizing \gls{SVD} compression is investigated. This decomposition is applied to the \gls{MLP}, self-attention and cross-attention matrices within a transformer block. To evaluate the efficiency of the \gls{SVD} compression method against the structural block removal method, a representative operating point is chosen. The rank for the \gls{MLP} is set to $r_\text{MLP} = 512$ corresponding to a compression of approximately $44\%$ of its parameters and the ranks for the self- and cross-attention matrices are fixed to $r_\text{self-attn} = r_\text{cross-attn} = 128$ representing a parameter reduction of approximately $78\%$. This serves as a starting point. Later, the choice of compressing attention matrices more aggressively than the \gls{MLP} matrices is further investigated (see sec. \ref{sec:MLPAttnDiffCompRatios}). \\
  ...
  For a qualitative comparison of both methods, Fig. \ref{fig:Example_Images_SVD_vs_Removal} provides a set of example images for compression ratios ranging from 90\% to 50\%. The advantage of the \gls{SVD} compression method becomes particularly evident in the high-compression regimes (60\% and 50\% compression ratios). Its images contain more details, exhibit fewer artefacts and are more closely resembel to the original image. Specifically, column two demonstrates that the woman's face is accurately preserved up to 60\% compression ratio when applying the \gls{SVD} compression technique. Moreover, column one shows that in high-compression regimes, complete block removal can lead to severe deviations compared to the original image, in contrast to the \gls{SVD} compression technique. Additionally, column five reveals that the images generated via complete block removal exhibit reduced sharpness and the plants seem to be blurry compared to those produced by \gls{SVD} compression.\\
   Interestingly, at a 90\% compression ratio, the \gls{SVD} compression technique leads to a valid but semantically altered image in column one, where the man's head is replaced by that of an alien. However, at different compression stages the model recovers and correctly generates a human head again. 
