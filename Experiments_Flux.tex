...
Second, as for PixArt-$\Sigma$ the qualitative block importance within Flux.1-dev is analyzed (Sec. \ref{sec:QualitativeBlockAnalysisFlux}). Third, the training-free performance of \gls{SVD}-based compression is evaluated and compared to complete block removal and EcoDiff \cite{EcoDiff} (Sec. \ref{sec:trainingFreeCompression}). In Sec. \ref{label} the main results and the comparison with competitors are reported, followed by the application of the method to a step-distilled \gls{DiT}, namely Flux.schnell (Sec. \ref{}). In addition, Sec. \ref{label} investigates the distribution of removed parameters over a varying number of blocks, whereas Sec. \ref{label} demonstrates the effectiveness of importance-aware block selection over random or uniform selection. Finally, Sec. \ref{label} provides an additional learning-free approach, GRASP (Sec. \ref{sec:GRASP}) which is transfgered from \gls{LLM}s to \gls{DiT} for potential future research.
...
\end{figure}
	...
	\caption[Qualitative Examples Importance and Random Block Selection Flux.1-dev]{Example images of the 49\% models (5.8B parameters) obtained by random and importance-based block selection.}
