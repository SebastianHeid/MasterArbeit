...
Fig. \ref{fig:BlockAnalysisCompressionSingle} and Fig. \ref{fig:BlockAnalysisCompressionDouble} present visual results for compressing individual blocks by 60\% rather than removing them entirely. Similar to PixArt-$\Sigma$, blocks whose removal leads to severe degradation in image quality, can be compressed without significant loss of image quality, e.g. single-stream blocks 35 and 38 or double-stream blocks 1, 2 and 19. The only double-stream block which appears to be highly critical and even compression exerts a substantial impact on the generated image quality is double-stream block number 3, whose compression results in visible stripes. This suggests that block 3 serves as a critical bottleneck for high-frequency processing. \\\\
Ultimately, the visual robustness of the compressed double-stream blocks strongly aligns with the parameter-weighted \gls{CMMD} predictions displayed in Fig. \ref{label}.
...
\end{figure}
	...
	\caption[Flux.1-dev Analysis Block Compression - Single-Stream Blocks]{Qualitative analysis of the impact of compressing individual single-stream blocks from the Flux.1-dev architecture by 60\%.}
	\label{fig:BlockAnalysisCompressionSingle}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{images/Experimente/Flux/Block_Analysis/BlockCompressionDouble.pdf}
	\caption[Flux.1-dev Analysis Block Compression - Double-Stream Blocks]{Qualitative analysis of the impact of compressing individual double-stream blocks from the Flux.1-dev architecture by 60\%.}
...
Nevertheless, while all methods effectively reduce the VRAM consumption during inference, standard implementations of \gls{SVD} and \gls{GRASP} do not inherently reduce the inference time as EcoDiff. This is because low-rank decomposition replaces one large matrix by two smaller matrices which doubles the number of matrix multiplications.
	...
	\begin{table}[H]
		...
		\begin{tabular}{@{}lcccc@{}}
			...
			\multirow{3}{*}{80\%}  
			& EcoDiff & 23.633 & 0.1873 &  49.12 \\
			& SVD & 30.45 & 0.585 & 79.84 \\
			& GRASP  & \textbf{30.82} & \textbf{0.623} & \textbf{81.19} \\ \midrule
			
			\multirow{3}{*}{70\%}  
			& EcoDiff & 7.26 &  & 17.02  \\
			& SVD & 28.87 & 0.455 & 73.13 \\
			& GRASP  & \textbf{30.08} & \textbf{0.490
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{images/Experimente/Flux/Training_Free/GRASP_DPG.pdf}
