...
\begin{table}[H]
	\centering
	\caption[Flux Finetuning Hyperparameters]{Configuration details for the compressed model retraining, including optimizer settings and flow matching specific parameters. These parameters represent the configuration setup for the training using the git repository \cite{kohya_ss_sd_scripts_2024}.}
...
The goal of this experiment is to investigate how the parameters to be removed should best be distributed across the model. Three different scenarios are investigated where Flux.1-dev is pruned by 10\% in each iteration with varying number of modifid blocks: 
...
\end{itemize}
Note, that the number of blocks does not specify if double- or single-stream blocks are compressed. This depends completely on the importance ranking of the blocks.
